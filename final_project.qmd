...
Below, we have pivoted the `year` variable in both datasets to be a single column, as opposed to the data frames having a column representing a year.
...
The `clean_long_df` will have 4 total columns: `country`, `year`, `life_expectancy`, and `gdp_per_capita`.
