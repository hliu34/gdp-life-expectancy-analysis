...
Before running the linear regression, summarising the data to one x value and one y value per country in the year 1945 would be a great way to simplify the regression model and ensuring each country is represented consistently. We decided to investigate 1945 because this is the year the Second World War ended.

The linear regression contains two variables of interest: average GDP per capitia for each country (explanatory variable: $X$) and average life expectancy for each (response variable: $Y$).
